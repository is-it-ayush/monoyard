...
export function isInstrumentArray(arr: any[]): arr is Instrument[] {
  return arr.every((item) => {
    return item.name !== undefined && item.hardware_id !== undefined && item.network_type !== undefined;
  });
}
...
export function isDriverGroupArray(arr: any[]): arr is DriverGroup[] {
  return arr.every((item) => {
    return item.name !== undefined && item.description !== undefined && item.folder_name !== undefined;
  });
}
